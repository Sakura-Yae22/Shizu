...
import { DiscordStatuslogs, DiscordStatuslogs as GuildModel } from '../mongoose/schemas/discord-status';
import Bot from '../client/Client';
import { StatuspageUpdates, Incident, ComponentStatus, IncidentStatus, Indicator, IncidentUpdates } from 'statuspage.js';
import settings from '../settings';
import { Guild, Webhook, TextChannel, User, MessageEmbed, Collection } from 'discord.js';
...
const Status_cache = new Collection<string, Status_CacheType>();
...
export default async function (client: Bot) {
    ...
    s.on('incident_update', async i => {
        ...
        for (const result of results) {
            ...
            const channel = await guild.channels.cache.get(`${BigInt(result.channelId)}`) as TextChannel
            if (!channel) {
            ...
            let ss
            if ((ss = Status_cache.find(a => a.incident === i.id && a.guildId === guild.id))) {
            ...
            if (!ss) {
                message = await hook.send({
                    ...
                    avatarURL: 'https://cdn.discordapp.com/attachments/815589214057529345/851385503822905354/9ed91074a5368ad9b394081408c3963e.png',
                    embeds: [embed]
                })
            } else {
                let check = false
                const messag = await channel.messages.fetch(`${BigInt(ss.msg_id)}`).catch(() => {
                    check = true
                })
                if (check) continue
                if (!messag) continue
            ...
            // hook.send(embed)
            console.log(`Guild Id: ${guild.id} & channel Id: ${channel.id} & message Id: ${message.id} & incident Id: ${i.id} & Update Id: ${update.id} has been sent an update of the discord Status`)
        }
    })
    s.on('start', async (...args) => {
        ...
        console.log(
            `Checking ${st.page.name} (${st.page.id}) for updates every ${s.interval / 1000
            }s`
        );
...
async function webhook(channel: TextChannel, guild: Guild, client: Bot): Promise<Webhook | void> {
    const arr: Webhook[] = [];
    let check = false;
    if (!channel.permissionsFor(guild?.me!).has('MANAGE_WEBHOOKS')) {
        ...
        await owner.send(`I wasnt able to get a webhook from the channel with the id ${channel.id} for the mod Logs\nI have **reset** the Mod Logs\nI dont have Perms for manage webhooks, pls make sure I have that permission`).catch(() => {
            return;
        });
        return
    }
    ...
    webhook.forEach(async (webhook: Webhook) => {
        if ((webhook.owner instanceof User) && webhook.owner.id === client.user.id) arr.push(webhook)
        else return;
    })
    if (!arr[0]) {
        if (webhook.size !== 10) {
            const web = await channel.createWebhook('Akio Logger', {
                ...
                const owner = await guild.fetchOwner()
                await owner.send(`I wasnt able to get a webhook from the channel with the id ${channel.id} for the mod logs\nI have **reset** the Mod Logs`).catch(() => {
                    return;
                });
                return
            })
...
async function Embed(i: Incident): Promise<MessageEmbed> {
    const formatFields = (update: IncidentUpdates) => {
        return {
            ...
            value: update.body.length > 1024 ? trim(update.body) : update.body ?? 'No Info Available',
        };
    }
    ...
    const embed = new MessageEmbed()
        .setAuthor(`Discord Status`, `https://discord.com/assets/2c21aeda16de354ba5334551a883b481.png`, `https://discord.gg/b7HzMtSYtX`)
        .setURL(`${i.shortlink}`)
