...
async function log(guild: Guild | any, client: Bot): Promise<void | Webhook> {
    ...
    if (!arr[0]) {
        if (webhook.size !== 10) {
            const web = await channel.createWebhook('Shizu Logger', {
