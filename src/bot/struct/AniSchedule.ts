...
export class Anischedule {
  ...
  info: { mediaFormat: { TV: string; TV_SHORT: string; MOVIE: string; SPECIAL: string; ONA: string; OVA: string; MUSIC: string; MANGA: string; NOVEL: string; ONE_SHOT: string; }; months: string[]; weeks: string[]; defaultgenres: string[]; langflags: { lang: string; flag: string; }[]; };
...
*/
  fetch(query: any, variables: any): Promise<unknown> {
 ...
 */
  getAllWatched(): Promise<Array<any>> {
    return new Promise(async resolve => {
      const list = await watchList.find({ _id: { '$in': [...this.client.guilds.cache.keys()] } }).catch(err => err);
      // const list = await this.client.database['GuildWatchlist'].find({ _id: { '$in': [...this.client.guilds.cache.keys()]}}).catch(err => err);
      if (list instanceof Error) return (console.log(`\x1b[35m[SHARD_${this.client.shard?.ids[0]}] \x1b[31m[Shizu]\x1b[0m: ${list.message}`));
  ...
  getAnnouncementEmbed(entry, date): MessageEmbed {
    ...
    return new MessageEmbed()
      ...
      .setFooter([
        ...
        `${entry.media.duration ? `Duration: ${moment.duration(entry.media.duration * 60, 'seconds').format('H [hr] m [minute]')}  ` : ''}`,
  ...
  */
  async handleSchedules(nextDay: any, page?: any) {
    ...
    if (!watched || !watched.length) {  //Retry in 1 minute if database fetching fails due to some error.
      ...
      return console.log(`\x1b[35m[SHARD_${this.client.shard?.ids.join(' ')}] \x1b[33m[Shizu]\x1b[0m: Missing Data from Database.\nNo lists were found on the database. Please ignore this message if this is the first time setting the bot.`);
    }
    ...
    if (res.errors) {
      ...
      return console.log(`\x1b[31m[Shizu]\x1b[0m: FetchError\n${res.errors.map(err => err.message).join('\n')}`);
    }
    ...
    for (const entry of res.data.Page.airingSchedules.filter(e => !this.queuedNotifications.includes(e.id))) {
      ...
      console.log(`\x1b[35m[SHARD_${this.client.shard?.ids.join(' ')}] \x1b[32m[Shizu]\x1b[0m: Episode \x1b[36m${entry.episode}\x1b[0m of \x1b[36m${entry_t}\x1b[0m airs in \x1b[36m${date}\x1b[0m.`);
 ...
 */
  async makeAnnouncement(entry, date) {
    ...
    for (const guild of list.filter(x => x.data.includes(entry.media.id))) {
      const channel = this.client.channels.cache.get(guild.channelId) as TextChannel
      const isValCh = channel instanceof TextChannel;
      const reqperm = [Permissions.FLAGS.SEND_MESSAGES, Permissions.FLAGS.EMBED_LINKS];
      const entry_t = Object.values(entry.media.title).filter(Boolean)[0];
      ...
      if (!isValCh || !channel.permissionsFor(channel.guild?.me!).has(reqperm)) {
        console.log(`\x1b[35m[SHARD_${this.client.shard?.ids.join(' ')}] \x1b[31m[Shizu]\x1b[0m: Announcement for ${entry_t} has \x1b[31mfailed\x1b[0m in ${dstintn} because ${freason}`);
        continue;
      }

      await channel.send({ embeds: [embed] })
        .then(message => console.log(`\x1b[35m[SHARD_${this.client.shard?.ids.join(' ')}] \x1b[32m[Shizu]\x1b[0m: Announcing new episode for \x1b[36m${entry_t}\x1b[0m to \x1b[36m${message.guild?.name}\x1b[0m`))
