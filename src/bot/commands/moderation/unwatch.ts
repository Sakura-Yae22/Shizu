...
abstract class UnWatchCommand extends Command {
    constructor() {
        super({
            ...
            usage: '<prefix>unwatch <\'Anilist/Mal link | releasing | not-yet-released | all\'>',
            category: 'mods',
            cooldown: 10,
            ownerOnly: false,
            guildOnly: true,
    ...
    public async exec(message: Message, links: string[], prefix: string) {
        try {
            ...
            const options: any[] = []
            let fetchedIDs: any[] = [], fetchedEntries: any[] = [], isStatusQuery = false;
            ...
            if (fetchedIDs.length === 1) {
                const ENTRY = String(`[**${Object.values(fetchedEntries[0].title).filter(Boolean)[0]}**](${fetchedEntries[0].siteUrl})`);
                ...
                return message.channel.send({
                    embeds: [
                        new MessageEmbed()
                            ...
                            .setDescription(String(tbd.length ? `${EMOJI} Successfully removed ${ENTRY}` : `Failed to remove ${ENTRY}\nThis anime is not on your watchlist!`))
            ...
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        ...
                        .setTitle(`${fetchedIDs.length} anime has successfully been processed.`)
                        .setFooter(`${DICT.UNWATCH}`)
                ]
            });
        }
