    ...
    public async exec(message: Message, args: string[]) {
        ...
        try {
            ...
            const confirmationEmbed = new MessageEmbed().setColor('RANDOM')
                ...
                .setTitle(`Are you sure you want to Kick ${target.user.tag}?`).setDescription(`${reason}`)
            ...
            collector.on('end', async collected => {
                ...
                if (hello.customId === 'yes_kick') {
                    ...
                    const kickEmbed = new MessageEmbed().setColor('RED')
                        .setAuthor(message.author.username, message.author.displayAvatarURL({
                            dynamic: true
                        }))
                        .setTitle(`Kicked ${target.user.tag}`).addField(`Reason`, `${reason}`).setTimestamp();
                    await target.send({
                        embeds: [new MessageEmbed().setTitle('Kick Alert').setColor('RED').addField('Guild Name', `${message.guild?.name}`).addField('Mod who executed', `${message.author.tag}`).addField('Reason', `${reason}`).setTimestamp()],
                        content: `Kick alert`,
                    }).catch(() => {
                        return;
                    });
                    ...
                    const cancelEmbed = new MessageEmbed().setColor('GREEN')
                        ...
                        .setTitle(`Cancelled Kicking ${target.user.tag}!`).setDescription(`Escaped **Kick** but the reason for the **ban** select was ${reason}`);
                    await mes.edit({
                        components: [],
                        embeds: [cancelEmbed]
                    });
