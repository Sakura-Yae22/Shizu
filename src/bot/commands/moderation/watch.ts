...
abstract class WatchCommand extends Command {
    constructor() {
        super({
            ...
            usage: '<prefix>watch <\'Anilist/Mal link | releasing | not-yet-released\'>',
            category: 'mods',
            cooldown: 10,
            ownerOnly: false,
            guildOnly: true,
    ...
    public async exec(message: Message, links: string[], prefix: string) {
        try {
            ...
            if (document instanceof Error) return message.channel.send({
                content: `A error has occurred! Pls report this to the devs`,
                components: [row]
            })
            if (!document) return message.reply({
            ...
            let fetchedIDs: string[] = [], fetchedEntries: any[] = [];
            ...
            for (const option of options) {
                ...
                do {
                    ...
                    const { errors, data } = await this.client.anischedule.fetch(watch, option);
            ...
            if (fetchedIDs.length === 1) {
                const ENTRY_TITLE = `[**${Object.values(fetchedEntries[0].title).filter(Boolean)[0]}**](${fetchedEntries[0].siteUrl})`
                const TIMEUNTILAIR = duration(fetchedEntries[0].nextAiringEpisode?.timeUntilAiring, 'seconds').format(`d [${DICT['DAY(S)']}] h [${DICT['HOUR(S)']}] m [${DICT['MINUTE(S)']}]`)
                const EPISODENUM = ordinalize(fetchedEntries[0].nextAiringEpisode?.episode)
                ...
                if (['FINISHED', 'CANCELLED', 'HIATUS'].includes(fetchedEntries[0].status)) {
                    ...
                    if (fetchedEntries[0].status === 'FINISHED') description += "This anime has already finished airing!"
                    else if (fetchedEntries[0].status === 'CANCELLED') description += "This anime has been cancelled!"
            ...
            const entries = fetchedIDs.slice(0, 20).map(id => {
                ...
                const title = truncate(Object.values(entry.title).filter(Boolean)[0], 40, '...');
                ...
                return String(`${added}\u2000\u2000\`[ ${idfmt} ]\` [**${title}**](${entry.siteUrl})`);
            });
