...
abstract class MuteRoleCommand extends Command {
	constructor() {
		super({
			...
			clientPermissions: ['ADD_REACTIONS', "USE_EXTERNAL_EMOJIS", "MANAGE_ROLES"]
		});
	}

	// tslint:disable-next-line: promise-function-async
	public async exec(message: Message, args: string[]) {
        const checkdata = await muterolschema.findOne({
        ...
        if (!checkdata) {
            ...
            const target = message.mentions.roles.first() || await message.guild?.roles.cache.get(`${BigInt(args[0])}`);
            if (!target) return message.channel.send({
                content: `Can\\'t find the role u specified`
            });
            await new muterolschema({
                guildId: message.guild?.id,
                muteRole: target.id
            }).save()
            await message.reply({
