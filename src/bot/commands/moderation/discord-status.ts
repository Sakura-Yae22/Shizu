...
import { CategoryChannel, Message, StageChannel, TextChannel, VoiceChannel } from 'discord.js';
...
abstract class setDiscordStatusCommand extends Command {
    ...
    public async exec(message: Message, args: string[]) {
        try {
            ...
            if (!Data) {
                const cid = message.mentions.channels.first() ?? await message.guild?.channels.cache.get(`${BigInt(args[0])}`)
                if (!cid) {
                    return message.reply(`Could\\'nt find a channel! Please provide a valid Id`)
                }
                if (cid instanceof CategoryChannel) {
                    return message.reply(`This is not a valid channel, This is is a category tagged\nMake sure this this is a text channel`)
                }
                if (cid instanceof VoiceChannel) {
                    return message.reply(`This is not a valid channel, This is is a Voice channel tagged\nMake sure this this is a text channel`)
                }
                if (cid instanceof StageChannel) {
                    return message.reply(`This is not a valid channel, This is is a Stage Channel Tagged\nMake sure this this is a text channel`)
                }
                await new schema({
                    guildId: message.guild?.id,
                    channelId: cid.id
                ...
                if (cid instanceof TextChannel)
                    await cid.createWebhook('Shizu Discord Status', {
                        avatar: 'https://cdn.discordapp.com/attachments/815589214057529345/851385503822905354/9ed91074a5368ad9b394081408c3963e.png',
