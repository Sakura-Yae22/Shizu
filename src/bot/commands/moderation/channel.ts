...
abstract class ChannelCommand extends Command {
    constructor() {
        super({
            ...
            usage: '<prefix>channel <delete/create/clone> [if create= channel name == if delete = channel tag == if clone = channel tag]',
            category: 'mods',
            cooldown: 0,
            ownerOnly: false,
            guildOnly: true,
            ...
            clientPermissions: ['ADD_REACTIONS', "USE_EXTERNAL_EMOJIS", "MANAGE_MESSAGES", 'MANAGE_CHANNELS']
        });
    }
    ...
    public async exec(message: Message, args: string[], prefix: string) {
        try {
            const filter = interaction => interaction.customId === 'yesc' || interaction.customId === 'noc' && interaction.user.id === message.author.id;
            if (args[1] === message.channel.id) return message.reply({
            ...
            switch (args[0]) {
                case 'delete':
                    ...
                    const target = message.mentions.channels.first() || await message.guild?.channels.cache.get(g)
                    if (!target) return message.reply({
                        content: `Channel not valid`
                    })
                    if (message.guild?.rulesChannelId === target.id) return message.reply({
                        content: `Rules channels cannot be deleted`
                    })
                    if (message.guild?.systemChannelId === target.id) return message.reply({
                    ...
                    const confirmationEmbed = new MessageEmbed().setColor('RANDOM')
                        ...
                        .setTitle(`Channel deletion request`).setDescription(`Do u want to delete ${target}`);
                    ...
                    collector.on('end', async collected => {
                        ...
                        if (hello.customId === 'yesc') {
                            await target.delete()
                            const dell = new MessageEmbed().setTitle('Channel Deleted : )').setTimestamp().setColor('RANDOM').setAuthor(message.author.username, message.author.displayAvatarURL({
                                dynamic: true
                            }))
                            await mes.edit({
                                components: [],
                            ...
                            const cancelEmbed = new MessageEmbed().setColor('GREEN')
                                ...
                                .setTitle(`Cancelled deleting of the channel!`).setDescription(`Escaped **Deletion** of ${target}`);
                            await mes.edit({
                                components: [],
                                embeds: [cancelEmbed]
                            });
                    ...
                    break;
                case 'create':
                    const confirmationEmbed2 = new MessageEmbed().setColor('RANDOM')
                        .setAuthor(message.author.username, message.author.displayAvatarURL({
                            dynamic: true
                        }))
                        .setTitle(`Channel Creation request`).setDescription(`Do u want to create a channel named ${args[1]}`)
                        .setThumbnail(`https://cdn.discordapp.com/attachments/831552576180322305/848846833845141554/294766366044211.png`);
                    ...
                    collector2.on('end', async collected => {
                        const hello = collected.first() as MessageComponentInteraction;
                        if (!hello) {
                            const hell = new MessageEmbed().setTitle('Time limited to 15 sec').setFooter('You did not react to the buttons within the time alloted').setColor('DARK_BUT_NOT_BLACK').setDescription('Please select One of the following buttons with these emojis').addField('The Yes Emoji', '<a:tick_yes:835437429288468521>').addField('The No Emoji', '<:tick_no:835440115706888195>'); await mes2.edit({
                                components: [],
                                embeds: [hell]
                            });
                            return;
                ...
                case 'clone':
                    ...
                    const target3 = message.mentions.channels.first() || message.guild?.channels.cache.get(g2)
                    if (!target3) return message.reply(`Channel not valid`)
                    const confirmationEmbed3 = new MessageEmbed().setColor('RANDOM')
                        .setAuthor(message.author.username, message.author.displayAvatarURL({
                            dynamic: true
                        }))
                        .setTitle(`Channel Clone request`).setDescription(`Do u want to Clone ${target3}`)
                        .setThumbnail(`https://cdn.discordapp.com/attachments/831552576180322305/848846833845141554/294766366044211.png`);
                    ...
                    collector4.on('end', async collected => {
                        ...
                        if (!hello3) {
                            const hell3 = new MessageEmbed().setTitle('Time limited to 15 sec').setFooter('You did not react to the buttons within the time alloted').setColor('DARK_BUT_NOT_BLACK').setDescription('Please select One of the following buttons with these emojis').addField('The Yes Emoji', '<a:tick_yes:835437429288468521>').addField('The No Emoji', '<:tick_no:835440115706888195>');
                        ...
                        if (hello3.customId === 'yesc') {
                            ...
                            const cancelEmbed3 = new MessageEmbed().setColor('GREEN')
                                .setAuthor(message.author.username, message.author.displayAvatarURL({
                                    dynamic: true
                                }))
                                .setTitle(`Cancelled Cloning of text channel!`).setDescription(`Escaped **Cloning** of ${target3}`);
                            await mes3.edit({
                                components: [],
                                embeds: [cancelEmbed3]
                            });
                    ...
                    break;
                default:
                    message.reply(`Tell me one of the following \`\`\`delete = ${prefix}channel delete <channel-tag>\ncreate = ${prefix}channel create <channel-name>\nclone = ${prefix}channel clone <channel-tag>\`\`\``)
            }
        } catch (e) {
