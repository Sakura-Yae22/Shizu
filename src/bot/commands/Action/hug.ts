...
abstract class HugCommand extends Command {
    constructor() {
        super({
            name: 'hug',
            aliases: [],
            description: 'Hug people : ) [NSFW should be reported immediately and the command should be be disabled]',
    ...
    public async exec(message: Message, args: string[]) {
        try {
            ...
            const embed = new MessageEmbed().setImage(`${url}`).setDescription(`Aww!! ${message.author} hugged ${target}`).setColor(`#FFC0CB`)
