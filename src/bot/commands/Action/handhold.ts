...
abstract class hHoldCommand extends Command {
    constructor() {
        super({
            name: 'hand-hold',
            aliases: ['hhold'],
            description: 'Hand Hold ur friends the discord way : ) [NSFW should be reported immediately and the command should be be disabled]',
    ...
    public async exec(message: Message, args: string[]) {
        try {
            ...
            const embed = new MessageEmbed().setImage(`${url}`).setDescription(`Aww!! ${message.author} is handholding ${target}`).setColor(`#FFC0CB`)
