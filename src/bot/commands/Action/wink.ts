...
abstract class WaveCommand extends Command {
    constructor() {
        super({
            name: 'wink',
            aliases: [],
            description: 'Wink at ur friends the discord way : ) [NSFW should be reported immediately and the command should be be disabled]',
    ...
    public async exec(message: Message, args: string[]) {
        try {
            ...
            const embed = new MessageEmbed().setImage(`${url}`).setDescription(`Aww!! ${message.author} is winking ${target}`).setColor(`#FFC0CB`)
