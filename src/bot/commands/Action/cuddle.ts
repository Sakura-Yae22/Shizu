...
abstract class CuddleCommand extends Command {
    constructor() {
        super({
            name: 'cuddle',
            aliases: [],
            description: 'Cuddle people : ) [NSFW should be reported immediately and the command should be be disabled]',
    ...
    public async exec(message: Message, args: string[]) {
        try {
            ...
            const embed = new MessageEmbed().setImage(`${url}`).setDescription(`Aww!! ${message.author} cuddled ${target}`).setColor(`#FFC0CB`)
