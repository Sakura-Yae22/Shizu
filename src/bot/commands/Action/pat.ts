...
abstract class PatCommand extends Command {
    constructor() {
        super({
            name: 'pat',
            aliases: [],
            description: 'Pat ur friends the discord way : ) [NSFW should be reported immediately and the command should be be disabled]',
    ...
    public async exec(message: Message, args: string[]) {
        try {
            ...
            const embed = new MessageEmbed().setImage(`${url}`).setDescription(`Aww!! ${message.author} patted ${target}`).setColor(`#FFC0CB`)
