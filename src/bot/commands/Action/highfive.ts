...
abstract class HighFiveCommand extends Command {
    constructor() {
        super({
            name: 'highfive',
            aliases: [],
            description: 'High five ur friends the discord way : ) [NSFW should be reported immediately and the command should be be disabled]',
    ...
    public async exec(message: Message, args: string[]) {
        try {
            ...
            const embed = new MessageEmbed().setImage(`${url}`).setDescription(`Aww!! ${message.author} is giving a highfive to ${target}`).setColor(`#FFC0CB`)
