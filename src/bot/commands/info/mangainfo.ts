						...
						this.client.kitsu.searchManga(search).then(async result => {
								...
								result.forEach(async (manga) => {
										const embed = new MessageEmbed()
												...
												.setTitle(`${manga.titles.english ? manga.titles.english : search}`).setURL(`https://kitsu.io/manga/${manga.slug}`)
												.setDescription(manga.synopsis.replace(/<[^>]*>/g, '').split('\n')[0])
												.addField('❯\u2000\Information', `•\u2000\**Japanese Name:** ${manga.titles.romaji}\n\•\u2000\**Age Rating:** ${manga.ageRating ? manga.ageRating : '`N/A`'}\n\•\u2000\**Chapters:** ${manga.chapterCount ? manga.chapterCount : '`N/A`'}`, false)
