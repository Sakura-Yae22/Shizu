	...
	public async exec(message: Message, _args: string[]) {
		try {
			...
			const hell2 = new MessageEmbed()
				...
				.setThumbnail(this.client.user.displayAvatarURL())
				.setColor('BLUE')
				.addField('**Platform**', `${process.platform}(${os.arch()})`).addField('**CPU**', `
			...
			const hell3 = new MessageEmbed()
				.setTitle('Core Info')
				.addField('**Developers**', '<@!773193286776389653>, <@!819196421021761567>', true)
				.addField('**Owners**', '<@!518738198982295564>, <@!719807591869317201>', true)
				.setColor('BLUE')
				.setAuthor(message.author.tag, message.author.displayAvatarURL({
					dynamic: true
				}), `https://discord.com/users/${this.client.user?.id}`)
				.setURL(`https://discord.com/users/${this.client.user?.id}`)
				.addField('Invite me (Voidbots)', `[Click](https://voidbots.net/bot/${this.client.user?.id}/invite)`, false)
				.addField('Vote for me (Voidbots)', `[Click](https://voidbots.net/bot/${this.client.user?.id}/vote)`, false);
