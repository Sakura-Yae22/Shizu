...
abstract class WatchingCommand extends Command {
    ...
    public async exec(message: Message, _links: string[], prefix: string) {
        try {
            ...
            if (listings instanceof Error) return message.channel.send({
                content: `A error has occurred! Pls report this to the devs`,
                components: [row]
            })
            if (listings === null || !listings.data.length) return message.reply({
            ...
            do {
                const res = await this.client.anischedule.fetch(watch, { watched: listings.data, page });
                if (res.errors) {
            ...
            const _mapFn = (entry) => hyperlinkify(formatID(entry), truncate(entry.title.romaji, 42, '...'), entry.siteUrl);
            ...
            if (!channel || !channel.permissionsFor(message.guild?.me!).has([Permissions.FLAGS.SEND_MESSAGES, Permissions.FLAGS.EMBED_LINKS])) {
                if (!channel) {
