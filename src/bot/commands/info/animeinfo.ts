			...
			if (!search) {
				...
				await this.client.kitsu.searchAnime(aq()).then(async (result) => {
					...
					const embed = new MessageEmbed()
						.setColor('RANDOM').setFooter(`Requested By ${message.author.tag}`, message.author.displayAvatarURL({
							dynamic: true
						})).setTitle(`${anime.titles.english}`).setURL(`https://kitsu.io/anime/${anime.slug}`)
						...
						.addField('❯\u2000\Youtube', `•\u2000\[**Click Me**](https://youtube.com/watch?v=${anime.youtubeVideoId})`).setImage(anime.posterImage.original);
				...
				res.forEach(async (anime: { titles: { english: any; romaji: any; }; slug: any; showType: any; posterImage: { original: string; }; synopsis: string; ageRating: any; nsfw: any; averageRating: any; ratingRank: any; popularityRank: any; episodeCount: any; startDate: any; endDate: any; youtubeVideoId: any; }) => {

					const embed = new MessageEmbed()
						.setColor('RANDOM').setTitle(`${anime.titles.english ? anime.titles.english : search}`).setURL(`https://kitsu.io/anime/${anime.slug}`)
