...
abstract class SteamCommand extends Command {
		...
		public async exec(message: Message, args: string[]) {
				try {
						...
						if (!res || !res.total) {
								return message.channel.send(`\\❌ Could not find **${query}** on <:stream:845570572054036500> steam`);
						}
						...
						const platformLogo = { windows: '<:windows:845581565937319956>', mac: '<:ios:845583332347215902>', linux: '<:linux:845581879142514698>' };
						...
						const platforms = Object.entries(data.platforms).filter(([_platform, has]) => has)
								.map(([platform]) => {
										return {
												name: '\u200b', inline: true,
						...
						return message.channel.send({
								embeds: [new MessageEmbed()
										.setColor(0x101D2F)
										.setTitle(data.name)
										.setThumbnail('https://cdn.discordapp.com/attachments/836253852855697418/845579957609562112/767062357952167946.png')
										...
										.addFields([
												...
												{ name: '\u200b', value: truncate(decode(data.detailed_description.replace(/(<([^>]+)>)/ig, ' ')), 980) },
												{ name: 'Supported Languages', value: `\u2000${truncate(html2md(data.supported_languages), 997)}` },
												...platforms
										])
								]});
				} catch (e) {
