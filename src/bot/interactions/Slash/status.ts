...
abstract class DiscordStatusInteraction extends Interaction {
	...
	public async exec(interaction: CommandInteraction, args: any[]) {
		...
		const notVoiceComponents = summary.components?.filter(c => !voiceComponentIds?.includes(c.id))
		...
		switch (args[0].value) {
			case 'voice':
				const voiceComponents = voiceComponentIds?.map(c => summary.components?.find(a => a.id === c) || null)
					.filter(Boolean)
					.sort((x, y) => (x?.name! > y?.name! ? 1 : -1))
				content =
					'**Voice Status**\n' +
					voiceComponents?.map(c => c && `> ${getStatusEmoji(c.status)?.emoji} **${c.name}:** ${capitalize(statusToWords(c.status))}`)
			...
			default:
				content =
					...
					notVoiceComponents
						?.map(
							c =>
								`> ${getStatusEmoji(c.status)?.emoji} **${c.name}**: ${capitalize(
