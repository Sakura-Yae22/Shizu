...
import { afk as afkSchema } from '../../mongoose/schemas/afk-message';
...
abstract class MessageEvent extends Event {
	...
	public async exec(message: Message) {
		...
		if (message.mentions.members?.first()) { // If message mentions someone
					...
					if (message.author.id === userId) {
						...
						message.channel.send({
							embeds: [
								new MessageEmbed()
									.setColor(message.guild?.me!.displayColor)
									.setDescription('<a:bounce:831518713333022736> **Welcome back, I removed your afk**')
							]
						});
						return
					}
