...
abstract class MessageEvent extends Event {
	...
	public async exec(message: Message): Promise<void> {
		...
		let prefix = message.guild ? getPrefix(message.guild.id) : this.client.defaultprefix
		...
		if (commandName) {
			...
			if (command) {
				if (message.mentions.channels.first()) {
					...
					if (ch?.type === 'GUILD_PRIVATE_THREAD' || ch?.type === 'GUILD_NEWS_THREAD' || ch?.type === 'GUILD_PUBLIC_THREAD') {
						message.channel.send(`You have mentioned a channel which is a thread`)
						return
					}
				}
				...
				if (message.channel instanceof TextChannel) {
					...
					if (userPermissions?.length) {
						...
						if (missingPermissions.length) {
							message.channel.send({
								content: String(`Your missing these required permissions: ${missingPermissions.join(', ')}`)
							});
							return
						}
					}
					if (clientPermissions?.length) {
						...
						if (missingPermissions.length) {
							message.channel.send({
								content: String(`I\\'m missing these required permissions: ${missingPermissions.join(', ')}`)
							});
							return
						}
					}
				...
				if (command.requiredArgs && command.requiredArgs > args.length) {
					message.channel.send({
						content: String(`Invalid usage of this command, please refer to \`${prefix}help ${command.name}\``)
					});
					return
				}
				if (command.cooldown) {
					...
					if (timestamps?.has(message.author.id)) {
						...
						if (cooldown) {
							...
							if (now < expirationTime) {
								...
								message.channel.send({
									content: String(`Wait ${timeLeft.toFixed(1)} more second(s) before reusing the \`${command.name}\` command.`)
								});
								return
							}
						}
